  ...
  public func parseMode(fromCommandLine commandLine: CommandLineArgumentProviderProtocol) -> SpeculidApplicationMode {
    ...
    if arguments.count > 0 {
      if arguments.contains("--help") {
        ...
        let urls = arguments[arguments.index(after: index)...].compactMap {
          filePath -> URL? in
        ...
        guard let asset = dictionary["asset"], let image = dictionary["image"], let destination = dictionary["destination"] else {
          return .command(.unknown(arguments))
        }
        return .command(.initialize(asset: asset, image: image, destination: destination))
      } else {
        return .command(.unknown(arguments))
      }
