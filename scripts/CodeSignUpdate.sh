...
bundleIdentifierApplication="com.brightdigit.Speculid-Mac-App"
bundleIdentifierHelper="com.brightdigit.Speculid-Mac-Installer"
...
if [[ $( /usr/libexec/PlistBuddy -c "Print NSPrincipalClass" "${infoPlist}" 2>/dev/null ) == *Application ]]; then
    target="application"
else
    target="helper"
fi
